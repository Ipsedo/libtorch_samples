...
struct MNIST_ConvNet : torch::nn::Module {
    MNIST_ConvNet() {
        c1 = register_module("c1", torch::nn::Conv2d(torch::nn::Conv2dOptions(1, 16, {3, 3})));
        c2 = register_module("c2", torch::nn::Conv2d(torch::nn::Conv2dOptions(16, 32, {5, 5})));

        l1 = register_module("l1", torch::nn::Linear(32 * 2 * 2, 10));
    }

    torch::Tensor forward(torch::Tensor x) {
        x = torch::relu(torch::max_pool2d(c1->forward(x), {3, 3}, {3, 3}));
